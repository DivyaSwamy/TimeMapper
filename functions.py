...
def get_datafromDB(nearest_zipcode,time):
    #
    # Connect to Server
    connection = psycopg2.connect(user="divyaswaminathan",
                              password="physics241",
                              host="127.0.0.1",
                              port="5432",
                              database="mytransitproject")
    cursor = connection.cursor()

    if connection:
        print ('You are connected to Database mytransitproject')
    #    
    # Get Data    
    postgres_select_query = 'SELECT destination_zipcode, transit_time_now,transit_time_traffic FROM public."distanceMatrix" WHERE origin_zipcode= ' + str(nearest_zipcode) + " AND start_time = '" + str(time)+"'"
...
def get_airbnb_listings(place):
    ...
    d = {'lat':lat, 'lng':lng  ,'local_neighborhood': local_neighborhood,'price': price}
...
#    return geojson
    ...
    if not listings_df.empty:
        for i in range(0,len(listings_df)):
            my_string = ' {}, price: {}'.format(listings_df['local_neighborhood'][i], listings_df['price'][i])
            folium.Marker([listings_df['lat'][i], listings_df['lng'][i]], 
                          icon=folium.Icon(color='green', icon='home'),
                           popup= my_string).add_to(Mapit)
    else:
