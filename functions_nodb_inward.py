...
pickle_in = open('distance_matrix.pkl', 'rb')
distance_matrix = pickle.load(pickle_in)
#######################################################################################
...
def get_datafromDB(nearest_zipcode,time,distance_matrix):

    #zc = distance_matrix ['origin_zipcode'] == nearest_zipcode
    zc = distance_matrix ['destination_zipcode'] == nearest_zipcode
    
    tt = distance_matrix['start_time']== time
    ####
    # Extract rows based on input
    ####
    record = distance_matrix[zc & tt]
    #### Drop  columns and rename
    #drop_column =  ['day', 'distance','id', 'origin_zipcode', 'start_time']
    drop_column =  ['day', 'distance','id', 'destination_zipcode', 'start_time']

    record = record.drop(drop_column, axis=1).reset_index(drop = True)
    #record.rename(columns = {'destination_zipcode':'zipcode'}, inplace = True)
    record.rename(columns = {'origin_zipcode':'zipcode'}, inplace = True)
...
#     pl_lng = -118.339741
    ...
    record1 = get_datafromDB(nearest_zipcode,time,distance_matrix)
...
#                   
    ...
    for i in range(0,len(dfg)):
        ...
        mypopup = folium.Popup(folium.IFrame(my_string, width = 220, height = 60), max_width=2650)
